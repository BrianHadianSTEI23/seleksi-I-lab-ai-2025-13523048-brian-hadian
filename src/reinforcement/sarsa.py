...
class SARSAWumpus : 
    ...
    def updateQTable(self, lastState, reward, currentState, action) :
        ...
        # chose new action based on the currentState
        if np.random.rand() < self.epsilon:
            nextAction = self.action[np.random.randint(0, len(self.action))]
        else:
            # get the dict of move and its value in current time
            nextAction = max(self.qTable[self.currState], key=lambda a: self.qTable[self.currState][a])

        # determine the currQTable value based on that action
        currMaxQTable = (self.qTable[currentState][nextAction])
